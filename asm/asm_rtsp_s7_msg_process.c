...
int rtsp_s7_setup_res_encode(S7_SETUP_RES resmsg, char *setup_res_msg)
{
	...
	if(resmsg.err_code==200){
		sprintf(str,"RTSP/1.0 %d OK\r\n"
			...
			resmsg.err_code,resmsg.cseq,resmsg.session,resmsg.ondemandsessionid,resmsg.ss.client, resmsg.ss.destination, resmsg.ss.client_port, resmsg.as.ip, resmsg.as.downPort);			
	}
