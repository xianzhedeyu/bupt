                  ...
                  const bool  sscope )
{
    if ( ! _running )
    {
        ...
        if ((status = pthread_attr_setschedpolicy(  & thread_attr, SCHED_RR )) != 0)
            //std::cerr << "(TThread) create : pthread_attr_setschedpolicy ("<< strerror( status ) << ")" << std::endl;
        
        if ((status = pthread_attr_setschedparam(   & thread_attr, & t_param )) != 0)
            //std::cerr << "(TThread) create : pthread_attr_setschedparam ("<< strerror( status ) << ")" << std::endl;

        if ((status = pthread_attr_setinheritsched( & thread_attr, PTHREAD_EXPLICIT_SCHED )) != 0)
            //std::cerr << "(TThread) create : pthread_attr_setinheritsched ("<< strerror( status ) << ")" << std::endl;
#endif
