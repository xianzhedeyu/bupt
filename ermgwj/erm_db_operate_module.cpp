...
int DB_exect(sqlite3 *db, const char * sql, const int sql_type, const char * tablename)//----sql_type:0 DDL ;1 DML;2 create table.--- execute sql //
{
	char *eMsg;
...
int DB_create() {
	...
	rc = DB_exect(db, sql_cmd, 2, tmpstr);
...
int QAM_ADD(int infocheck, const qaminfo infocontent, int nextservercheck, const qamnextserver nextservercontent, int udpcheck, const qamudp udpcontent, int inputcheck, const qaminput inputcontent)
...
int QAM_SETUP_DOWN(const qamsdinfo qma_sd) {
	sqlite3 *db = NULL;
	int rc;
	int ncount = 0;
	char *eMsg;
	...
	if (qma_sd.udp_state == 4) {
		tmpstate = 0;
		sprintf(tmpstr, ",Qam_session=null,Ondemand_session=null,State_time=null,use_bw=null ");
	} else {
		tmpstate = qma_sd.udp_state;
		snprintf(tmpstr, sizeof(tmpstr), ",Qam_session='%s',Ondemand_session='%s',State_time='%ld' ,use_bw=%d ", qma_sd.qam_session, qma_sd.ondemand_session, time(NULL), qma_sd.use_bw);
	}

	memset(sql_cmd, 0x00, sizeof(sql_cmd));
	sprintf(sql_cmd, "update Qam_udp set udp_type=%d %s where qam_name='%s' and udp_port=%d  and Class_num=1;", tmpstate, tmpstr, qma_sd.qam_name, qma_sd.udp_port);
	rc = DB_exect(db, sql_cmd, 1, NULL);
 ...
 * 				availablebw（可用总带宽）
 * */
int QAM_SELECT(qamselectinfo *qam_info, int qam_numb) {
	qamselectinfo tmpselect[qam_numb];
	memset(&tmpselect, 0x00, sizeof(qamselectinfo) * qam_numb);

	//将 QAM 的信息 复制给了 临时的 tmpselect
	memcpy(&tmpselect, qam_info, sizeof(qamselectinfo) * qam_numb);
...
int QAM_DOWN_SELECT(qamselectinfo_down *qs) {

	qamselectinfo_down tmpselect;
	memset(&tmpselect, 0x00, sizeof(qamselectinfo_down));
	memcpy(&tmpselect, qs, sizeof(qamselectinfo_down));
	...
	/* qam_udp:
	 * 存放的已连接的 SS－> QAM 的信息*/
	sprintf(sql_cmd, "select count(*) from qam_udp where Ondemand_session='%s' ;", tmpselect.ondemand_session);
	sqlite3_get_table(db, sql_cmd, &azResult, &nrow, &ncolumn, &eMsg);
	int tmpnum;
	...
	if (tmpnum > 0) {
		memset(sql_cmd, 0x00, sizeof(sql_cmd));
		memset(tmpstr, 0x00, sizeof(tmpstr));
		sprintf(sql_cmd, "select qam_name,qam_session,use_bw from qam_udp where Ondemand_session='%s' ;", tmpselect.ondemand_session);
		sqlite3_get_table(db, sql_cmd, &azResult, &nrow, &ncolumn, &eMsg);
		...
		tmpselect.use_bw = atoi(azResult[4]);
	...
	memcpy(qs, &tmpselect, sizeof(qamselectinfo_down));
