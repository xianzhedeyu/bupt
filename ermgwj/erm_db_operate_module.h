...
char ondemand_session[MAX_STRING];
}qamsdinfo;

typedef struct _qamselectinfo_down
{
 char ondemand_session[MAX_STRING]; //in
 char qam_name[MAX_STRING];
 int use_bw;
...
int DB_exect(sqlite3 *db,const char * sql,const int sql_type,const char * tablename);
int DB_create();
int DB_initializtion();
int QAM_ADD(int infocheck, const qaminfo infocontent, int nextservercheck, const qamnextserver  nextservercontent ,
	int udpcheck, const qamudp udpcontent,int inputcheck,const qaminput inputcontent);
