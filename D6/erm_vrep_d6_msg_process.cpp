...
bool ParseOPEN(byte* VREP,OPEN &M_OPEN)                      //OPEN消息解析函数
{
	...
	//cout<<"===ParseOPEN===len: "<<M_OPEN.Len<<endl;
	//cout<<"===ParseOPEN===type: "<<M_OPEN.type<<endl;
	//cout<<"===ParseOPEN===version: "<<M_OPEN.version<<endl;
	//cout<<"===ParseOPEN===holdtime: "<<M_OPEN.holdtime<<endl;
	//cout<<"===ParseOPEN===parametersLen: "<<M_OPEN.parametersLen<<endl;
	//cout<<"===ParseOPEN===ParameterNum: "<<M_OPEN.ParameterNum<<endl;
	//cout<<"===ParseOPEN===VREP ID: ";
	unsigned char ascbuf[16];
	ChangeAscii2String((char*)ascbuf,(char*)M_OPEN.ID,4);
	cout<<ascbuf<<endl;
	//cout<<"===ParseOPEN===END "<<endl;
...
//	Routes M_Routes[256];
	...
	while(restlen>0)
	{
		...
		restlen=restlen-(6+routes[RNum].AddLen);
		char routes_add[MAX_STRING];
		memset(routes_add, 0x00, sizeof(routes_add));
		memcpy(routes_add, routes[RNum].Address, sizeof(routes[RNum].Address));
		RNum++;
		//cout<<"[AddFamily="<<routes[RNum].AddFamily<<" AppProtocol="<<routes[RNum].AppProtocol<<" Address="<<routes_add<<"]"<<endl;
	}
	Num=RNum;
}
...
{
	M_NextHopServer.AddLen=btos(Attr,4);
	int i=0;
	...
	memset(&M_NextHopServer.ZoneName, 0x00, sizeof(M_NextHopServer.ZoneName));
	//cout<<"===ParseNHS===SZNameLen="<<M_NextHopServer.ZoneNameLen<<endl;
	for(i=0;i<M_NextHopServer.ZoneNameLen;i++)
	{
		M_NextHopServer.ZoneName[i]=M_ZoneName[i];
		//cout<<M_NextHopServer.ZoneName[i];
	}
	//cout<<endl;
}
...
void ParseEdgeInput(byte* Attr,short Len,Input* EdgeInput,int &Num)    //解析参数EdgeInput
{
	short sumlen=0;
	int INum=0;
	cout<<"===ParseEdgeInput===IN===Len="<<Len;
	while(sumlen<Len)
	{
		int i=0;
		short HostLen=0;
        ...
        cout<<"[EdgeInput-"<<INum<<" SubnetMask="<<EdgeInput[INum].SubnetMask<<" Host="<<EdgeInput[INum].Host<<" MaxGroupBW="<<EdgeInput[INum].MaxGroupBW<<" EdgeInputGroup="<<EdgeInput[INum].GroupName<<"]"<<endl;
...
void ParseUDPMap(byte* Attr,short Len,StaticPorts* SPorts,DynamicPorts* DPorts,int &SNum,int &DNum)     //解析参数UDPMap
{
	...
	//cout<<"[SPortNum="<<SPortNum;
	//cout<<" DPortNum="<<DPortNum<<"]";
	...
	for(i=0;i<DPortNum;i++)
	{
		...
		//cout<<"[StartingPort="<<DPorts[i].StartingPort<<ends<<" StartingPID="<<DPorts[i].StartingPID<<ends<<" Count="<<DPorts[i].Count<<"]";
	}
	SNum=SPortNum;
	DNum=DPortNum;
}
...
void ParsePortID(byte *Port,PortID &M_Port)
{
	...
	int tmp=bytes2int(Port);
    //cout<<"[OutputPort="<<tmp<<" slotnumber="<<M_Port.slotnumber<<" portnumber="<<M_Port.portnumber<<" subinterface="<<M_Port.subinterface<<"]"<<endl;
}
	...
	for(int i=0;i<ANum;i++)
	{
		...
		switch(M_attr[i].AttrType)
		{
            ...
            case VREP_ID_D6_UPDATE_NextHopServer:
                ...
                //cout<<"=NextHopServer="<<ends;
                //cout<<"===ParseNextHopServer==="<<endl;
                break;
            ...
            case VREP_ID_D6_UPDATE_EdgeInput:
                ...
                //cout<<"===ParseEdgeInput===EdgeInputNum="<<M_UPDATE.EdgeInputNum<<endl;
                break;

            case VREP_ID_D6_UPDATE_QAMParameters:
                QAMParameters M_QAMP;
                memset(M_UPDATE.QAMP.Modmode, 0x00, sizeof(M_UPDATE.QAMP.Modmode));
                memset(M_QAMP.Modmode, 0x00, sizeof(M_QAMP.Modmode));
                ParseQAMP(M_attr[i].AttrValue,M_attr[i].AttrLen,M_QAMP);                       //解析参数QAMParameters
                M_UPDATE.QAMP=M_QAMP;
                //cout<<"=QAMParameters="<<ends;
                //cout<<"===ParseQAMParameters==="<<endl;
            ...
            case VREP_ID_D6_UPDATE_ServiceStatus:
                ...
                //cout<<"=ServiceStatus=["<<M_UPDATE.ServiceStatus<<"]"<<ends;
                //cout<<"===ParseServiceStatus===ServiceStatus="<<M_UPDATE.ServiceStatus<<endl;
                break;

            case VREP_ID_D6_UPDATE_MaxMpegFlows:
                M_UPDATE.MaxMpegFlows=bytes2int(M_attr[i].AttrValue);                          //解析参数MaxMpegFlows
                //cout<<"=MaxMpegFlows="<<ends;
                //cout<<"===ParseMaxMpegFlows===MaxMpegFlows="<< M_UPDATE.MaxMpegFlows<<endl;
                break;
            ...
            case VREP_ID_D6_UPDATE_NextHopServerAlternates:
                ...
                //cout<<"=NextHopServerAlternates="<<ends;
                //cout<<"===ParseNextHopServerAlternates==="<<endl;
                break;

            case VREP_ID_D6_UPDATE_OutputPort:
                //cout<<"=OutputPort="<<ends;
                ParsePortID(M_attr[i].AttrValue,M_UPDATE.OutputPort);                           //解析参数OutputPort
                //cout<<"===ParseOutputPort==="<<endl;
                break;
            ...
            case VREP_ID_D6_UPDATE_UDPMap:
                ...
                //cout<<"===ParseUDPMap===StaticPortsNum="<<M_UPDATE.SPortsNum<<"; DynamicPortsNum="<<M_UPDATE.DPortsNum<<endl;
                break;

            default:
                //TODO:未知代码参数处理
                //cout<<"===ParseAbnormolAttributes==="<<endl;
                return false;
			break;
		}
	}
...
bool ParseUPDATE(byte VREP[],UPDATE &M_UPDATE)                   //解析UPDATE消息函数
{
	...
	//cout<<"===ParseUPDATE===len: "<<M_UPDATE.Len<<endl;
	//cout<<"===ParseUPDATE===type: "<<M_UPDATE.type<<endl;
	cout<<"===ParseUPDATE===[len:"<<M_UPDATE.Len<<" AttributeNum:"<<M_UPDATE.AttributeNum<<"]"<<endl;
...
{
	OpenMessage[0]=(M_OPEN.Len>>8) & 0x00;
	//OpenMessage[0]=(M_OPEN.Len |  0x00)>>8;
	OpenMessage[1]=M_OPEN.Len | 0x00;
	OpenMessage[2]=0x01;
	OpenMessage[3]=(char)M_OPEN.version;
	//memcpy(&OpenMessage[3],&(M_OPEN.version & 0xFF),1);
	//OpenMessage[3]=0x02;
	OpenMessage[4]=0x00;
	//OpenMessage[5]=(M_OPEN.holdtime>>8) & 0xFF;
	OpenMessage[5]=(M_OPEN.holdtime & 0xFF)>>8;
	OpenMessage[6]=M_OPEN.holdtime & 0xFF;
	OpenMessage[7]=0x00;
	OpenMessage[8]=0x00;
	OpenMessage[9]=0x00;
        OpenMessage[10]=0x00;
	OpenMessage[11]=(char)M_OPEN.ID[0];
	OpenMessage[12]=(char)M_OPEN.ID[1];
	OpenMessage[13]=(char)M_OPEN.ID[2];
	OpenMessage[14]=(char)M_OPEN.ID[3];
	//OpenMessage[15]=(M_OPEN.parametersLen>>8) & 0xFF;
	OpenMessage[15]=(M_OPEN.parametersLen & 0xFF)>>8;
	OpenMessage[16]=M_OPEN.parametersLen & 0xFF;

	int sumlen=0;
	for(int i=0;i<M_OPEN.ParameterNum;i++)
	{
		//OpenMessage[17+sumlen]=(M_OPEN.parameters[i].ParameterType>>8) & 0xFF;
		OpenMessage[17+sumlen]=(M_OPEN.parameters[i].ParameterType & 0xFF)>>8;
		OpenMessage[18+sumlen]=M_OPEN.parameters[i].ParameterType & 0xFF;
		//OpenMessage[19+sumlen]=(M_OPEN.parameters[i].SubParametersLen>>8) & 0xFF;
		OpenMessage[19+sumlen]=(M_OPEN.parameters[i].SubParametersLen & 0xFF)>>8;
		OpenMessage[20+sumlen]=M_OPEN.parameters[i].SubParametersLen & 0xFF;
		for(int j=0;j<M_OPEN.parameters[i].SubParametersLen;j++)
			OpenMessage[21+sumlen+j]=(char)M_OPEN.parameters[i].ParameterValue[j];
		sumlen=sumlen+4+M_OPEN.parameters[i].SubParametersLen;
	}

	//cout<<"===PackageOpen===[";
        //int num;
        //for(num=0; num<=M_OPEN.Len; num++)
        //{
        //        cout<<OpenMessage[num]<<";";
        //}
        //cout<<"]"<<endl;

	//int num2=(int)M_OPEN.Len;
	//cout<"!!!!!!!"<<num2;
	//unsigned char ascbuf1[num2];
        //ChangeAscii2String((char*)ascbuf1,(char*)OpenMessage,num2);
        //cout<<"!!!!!!"<<ascbuf1<<endl;


	//unsigned char ascbuf[M_OPEN.Len];
	//memset(&ascbuf, 0x00, M_OPEN.Len);
        //ChangeAscii2String((char*)ascbuf,(char*)OpenMessage,M_OPEN.Len);
	//cout<<"===PackageOpen===[OpenMessage="<<ascbuf<<"]"<<endl;
	cout<<"===PackageOpen===END"<<endl;
	return true;
}
