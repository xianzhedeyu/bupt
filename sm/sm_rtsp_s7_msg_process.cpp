...
int rtsp_s7_setup_msg_encode(S7_SETUP_MSG msg,char *setup_msg)
{
	...
	sprintf(ss_info,"MP2T/DVBC/UDP;unicast;client=%s;destination=%s;client_port=%d;bandwidth=%llu",msg.ss.client,msg.ss.destination,msg.ss.client_port, msg.ss.bandwidth);
                ...
                while(subline !=  NULL) {
                    if(subheader = strtok_r(subline, "=", &ptr4)) {
                        if(strcmp(subheader, "client") == 0) {
                            ...
                            subheader_value = strtok_r(NULL, "=", &ptr4);
                            res->ss.bandwidth = atol(subheader_value);
                        }
                    }
...
//鍒涘缓sm鍙戠粰asm鐨刟nnounce response娑堟伅
